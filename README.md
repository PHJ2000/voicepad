...
- `음성 명령`으로 보내기, 삭제, 교체, 복사, 붙여넣기, 잘라내기, 현재 입력창 전체 비우기, 언어 전환, 미디어 제어 가능
- `백그라운드 시작` 지원
...
- 음성 명령
  ...
  - `esc`, `앞으로 감기`, `뒤로 감기`, `세 번 앞으로 감기`, `두 번 뒤로 감기`처럼 미디어 제어 명령도 사용할 수 있습니다.
...
- `esc`, `앞으로 감기`, `뒤로 감기`는 현재 포커스된 창에 방향키/esc 단축키를 보내는 방식이라 유튜브 같은 웹 플레이어에서 특히 잘 맞습니다.
